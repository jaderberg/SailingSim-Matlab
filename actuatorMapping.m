...
function actuatorMapping(hull, x)
    ...
    ip = [0 x(4) x(5)]';
    %ip = [0 0 0]';
    op = fminsearch(@orientationCost,ip,optimset('MaxFunEvals',9999,'MaxIter',9999),x,hull,maxActuatorz,minActuatorz);
    ...
    function cost = orientationCost(p, x, hull, max, min)
        ...
        for n=1:1:length(bestHeights)
            ...
            cost = cost - (1/10)*log(-u1) - (1/10)*log(-u2);
        end
        
        %differences in pairs must be the same
%         if sum(bestHeights) ~= 0
%             cost = cost - (1/10)*log(-(bestHeights(1)-bestHeights(2)-bestHeights(4)+bestHeights(3))) - (1/100)*log(-(bestHeights(2)-bestHeights(3)-bestHeights(1)+bestHeights(4)));
%         end
